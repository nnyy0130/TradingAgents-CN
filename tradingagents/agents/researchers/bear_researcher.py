...
def create_bear_researcher(llm, memory):
    def bear_node(state) -> dict:
        ...
        try:
            ...
            system_prompt = apply_compliance_guardrails(system_prompt, "bear_researcher")
            logger.info(f"✅ [空头研究员] 成功从模板系统获取提示词 (长度: {len(system_prompt)})")

        except Exception as e:
            logger.error(f"❌ [空头研究员] 从模板系统获取提示词失败: {e}")
            # 降级：使用硬编码提示词
            system_prompt = f"""你是一位偏空研究员，负责论证股票 {company_name}（股票代码：{ticker}）存在的风险与不利因素。
...
{build_research_only_closing(["请使用这些信息提供偏空研究论点，反驳看涨声明，并参与动态辩论。"])}"""
