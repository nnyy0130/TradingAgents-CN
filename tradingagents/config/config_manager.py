...
⚠️⚠️⚠️ DEPRECATED - 已过期 83 天 ⚠️⚠️⚠️
====================================
此模块已废弃，原计划移除日期：2026-03-31
当前日期：2026-06-23（已过期）
...
# 🔴 发出废弃警告（已过期）
warnings.warn(
    "ConfigManager is DEPRECATED and EXPIRED (planned removal: 2026-03-31, now 2026-06-23). "
    "Please use app.services.config_service.ConfigService instead. "
    "44 files still reference this module - migration required before removal!",
    DeprecationWarning,
    stacklevel=2
)
...
class ConfigManager:
    ...
    def _load_env_file(self):
        ...
        if env_file.exists():
            ...
            logger.info(f"🔍 [ConfigManager] 加载环境文件: {env_file}")
    ...
    def _init_mongodb_storage(self):
        ...
        try:
            ...
            # 🔧 当 MONGODB_CONNECTION_STRING 未设置时，自动构建并写入环境变量
            # 这样其他模块读取 MONGODB_CONNECTION_STRING 环境变量时也能获取到正确的连接字符串
            if not os.getenv('MONGODB_CONNECTION_STRING'):
                os.environ['MONGODB_CONNECTION_STRING'] = connection_string
                logger.info(f"🔍 [ConfigManager] 已自动构建 MONGODB_CONNECTION_STRING (长度: {len(connection_string)})")
